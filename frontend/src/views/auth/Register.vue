<template>
  ...
  >
    ...
    <form @submit.prevent="submit" class="space-y-4">
      ...
      <div>
        <label class="ml-label" for="register-email">{{ t('auth.email') }}</label>
        <input
          id="register-email"
          v-model="form.email"
          type="email"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label class="ml-label" for="register-password">{{ t('auth.password') }}</label>
      ...
      <button type="submit" class="ml-btn-primary w-full" :disabled="auth.loading" data-testid="register-submit">
        {{ auth.loading ? t('auth.creatingAccount') : t('auth.createAccountCta') }}
      </button>
    </form>
    ...
    <template v-if="googleEnabled">
      <div class="mt-6 flex items-center justify-between">
        ...
        <span class="text-xs text-center text-gray-500 uppercase">{{ t('common.or') }}</span>
        <span class="w-1/5 border-b lg:w-1/4"></span>
      </div>
    ...
    <template #footer>
      <p class="text-center text-sm text-ink-500">
        ...
        <router-link :to="loginLink" class="font-semibold text-brand-600 hover:text-brand-700">{{ t('auth.signIn') }}</router-link>
      </p>
    </template>
  </AuthShell>
</template>
