<template>
  <div class="min-h-screen bg-ink-50" data-testid="booking-page-root">
    ...
    <div class="max-w-4xl mx-auto">
      ...
      >
        <span class="mr-1">←</span> {{ auth.isVendorUser ? t('booking.backToDashboard') : t('booking.backToCommunity') }}
      </router-link>

      <div class="ml-card">
        <div class="mb-6">
          <span class="ml-badge bg-brand-100 text-brand-700">{{ t('booking.onboardingBadge') }}</span>
          <h1 class="mt-2 text-2xl font-extrabold text-ink-900 tracking-tight" data-testid="booking-onboarding-heading">
            {{ t('booking.startTitle') }}
          </h1>
        ...
        >
          <div class="flex items-start justify-between gap-3 mb-2">
            <p class="text-xs font-bold uppercase tracking-wider text-brand-700">{{ t('booking.selectedEvent') }}</p>
            <button
              type="button"
              class="ml-btn-ghost text-sm shrink-0"
              data-testid="booking-change-event-button"
              :aria-label="t('booking.changeEventAria')"
              @click="requestChangeEvent"
            >
              {{ t('booking.changeEvent') }}
            </button>
          </div>
          <h2 class="text-lg font-extrabold text-ink-900">{{ selectedEvent.title }}</h2>
          <dl class="mt-3 space-y-2 text-sm text-ink-700">
            <div class="flex gap-2">
              <dt class="w-20 shrink-0 font-semibold text-ink-500">{{ t('booking.date') }}</dt>
              <dd>{{ selectedEvent.dateLabel }}</dd>
            </div>
            <div class="flex gap-2">
              <dt class="w-20 shrink-0 font-semibold text-ink-500">{{ t('booking.time') }}</dt>
              <dd>{{ selectedEvent.time }}</dd>
            </div>
            <div class="flex gap-2">
              <dt class="w-20 shrink-0 font-semibold text-ink-500">{{ t('booking.location') }}</dt>
        ...
        <div v-if="!loadingEvents && !selectedEvent && (!routeEventId || eventLoadError)" class="mb-6">
          <label for="event-select" class="ml-label">{{ t('booking.selectEvent') }}</label>
          <select
            id="event-select"
          ...
          >
            <option value="" disabled>{{ t('booking.chooseUpcomingEvent') }}</option>
        ...
        <form v-if="!loadingEvents" @submit.prevent="submitBooking" class="space-y-4" data-testid="booking-form">
          ...
          >
            <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
              <div>
                <p class="font-bold text-brand-800">{{ t('booking.quickRebookTitle') }}</p>
                <p class="mt-1">
                  {{ t('booking.quickRebookBody') }}
                </p>
              </div>
              <button
                type="button"
          ...
          <div>
            <label class="ml-label">{{ t('booking.yourName') }}</label>
            <input v-model="userName" type="text" required class="ml-input" :placeholder="t('booking.namePlaceholder')" data-testid="booking-business-name" />
          </div>

          <div>
            <div class="flex items-center gap-2 mb-1">
              <label class="ml-label !mb-0">{{ t('booking.productsLabel') }}</label>
              ...
              <div class="group relative flex flex-col items-center">
                ...
                <div class="absolute bottom-full mb-2 hidden w-56 flex-col items-center group-hover:flex group-focus:flex group-focus-within:flex">
                  <span class="relative z-10 rounded-lg bg-ink-900 p-2.5 text-xs leading-relaxed text-white shadow-lg text-center">
                    {{ t('booking.productsTip') }}
                    <br/><br/>
                    {{ t('booking.productsTipExtra') }}
                  </span>
            ...
            <input
              ...
              required
              class="ml-input"
              :placeholder="t('booking.productsPlaceholder')"
          ...
          <div class="rounded-xl border border-ink-100 bg-ink-50/50 px-4 py-3">
            <label class="flex items-start gap-3 cursor-pointer">
              ...
              />
              <span>
                <span class="block text-sm font-semibold text-ink-800">{{ t('booking.saveDetails') }}</span>
                <span class="block text-xs text-ink-500 mt-1">
                  {{ t('booking.saveDetailsHint') }}
                </span>
              </span>
            </label>
          </div>
          ...
          >
            <div class="border-b border-ink-100 px-5 py-4">
              <h3 id="change-event-dialog-title" class="text-lg font-extrabold text-ink-900">
                {{ t('booking.changeEventDialogTitle') }}
              </h3>
            </div>

            <div class="px-5 py-4">
              <p class="text-sm leading-relaxed text-ink-700">
                {{ t('booking.changeEventDialogBody') }}
              </p>
            </div>
...
const loadVendorCategories = async () => {
  ...
  try {
    ...
    categoryLoadError.value = t('booking.ops.unableLoadCategories');
  } finally {
  ...
  try {
    ...
    if (id) {
      ...
      if (!bookingForm.vendor_category_id) {
        ...
        liveAnnouncement.value = t('booking.ops.suggestedCategorySelected');
...
const clearSelectedEvent = async () => {
  ...
  liveAnnouncement.value = t('booking.ops.selectAnotherEvent');
};
...
const loadSiteAvailability = async (eventId, { preserveSelection = false } = {}) => {
  ...
  if (!bookingForm.vendor_category_id) {
    resetSiteSelection();
    availabilityReadiness.value = t('booking.ops.selectCategoryFirst');
    return;
  }
  ...
  try {
    ...
    if (data.category_required) {
      ...
      availabilityDays.value = Array.isArray(data.operational_days) ? data.operational_days : [];
      availabilitySitePrice.value = data.site_price ?? data.event?.site_price ?? null;
      availabilityReadiness.value = data.readiness?.message || t('booking.ops.selectCategoryFirst');
      return;
    }
    ...
    availabilityReadiness.value = data.readiness?.status === 'no_compatible_sites'
      ? t('booking.ops.noSitesForCategory')
      : (data.readiness?.message || '');
    liveAnnouncement.value = availabilityRows.value.length
      ? t('booking.ops.availableSitesLoaded', {
          count: availabilitySites.value.filter((site) => site.is_selectable).length,
        })
      : availabilityReadiness.value;
  } catch (error) {
    if (requestToken !== availabilityRequestToken) return;
    ...
    if (error.response?.status === 422) {
      ...
      availabilityReadiness.value =
        ...
        || t('booking.ops.eventNotReadyForSites');
      if (error.response?.data?.error === 'EVENT_LAYOUT_NOT_READY') {
        layoutNotReadyForBooking.value = true;
        selectedSiteIds.value = [];
        availabilityReadiness.value = t('booking.ops.bookingNotOpen');
      }
      return;
    }

    resetSiteSelection();
    availabilityError.value = t('booking.ops.unableLoadSites');
  } finally {
...
const clearSavedPreference = async () => {
  ...
  try {
    ...
    toast.success(t('booking.toastSavedCleared'));
  } catch (error) {
    console.error('Failed to clear saved booking preference:', error);
    toast.error(error.response?.data?.message || t('booking.ops.clearSavedFailed'));
  } finally {
...
const chooseAnotherCategory = () => {
  bookingForm.vendor_category_id = '';
  liveAnnouncement.value = t('booking.ops.chooseAnotherCategory');
  requestAnimationFrame(() => {
...
const loadSelectedEventFromRoute = async () => {
  ...
  try {
    ...
    selectedEvent.value = null;
    selectedEventId.value = '';
    eventLoadError.value = t('booking.ops.eventUnavailable');
  }
};
...
watch(
  ...
  (next, prev) => {
    ...
    siteSelectionError.value = '';
    if (prev) {
      categoryChangeNotice.value = t('booking.ops.categoryCleared');
      toast.info(t('booking.ops.categoryCleared'));
      liveAnnouncement.value = hadSites
        ? t('booking.ops.categoryCleared')
        : t('booking.ops.categoryChangedLoading');
    }

    if (!next) {
      resetSiteSelection();
      availabilityReadiness.value = t('booking.ops.selectCategoryContinue');
      return;
    }
...
onMounted(async () => {
  ...
  try {
    ...
    toast.error(t('booking.toastEventsLoad'));
  } finally {
...
const handleBookingConflict = async () => {
  await refreshAvailabilityAfterConflict(t('booking.ops.siteConflict'));
};
...
const refreshAvailabilityAfterConflict = async (message) => {
  ...
  siteSelectionError.value = t('booking.ops.siteSelectionUpdated');
  liveAnnouncement.value = removedStaleSiteLabels.value.length
    ? `${siteSelectionError.value} ${t('booking.ops.removedSites', { labels: removedStaleSiteLabels.value.join(', ') })}`
    : siteSelectionError.value;
};
...
const submitBooking = async () => {
  if (!canSubmit.value || !selectedEvent.value) {
    toast.error(
      bookingForm.vendor_category_id
        ? t('booking.ops.selectEventAndSite')
        : t('booking.ops.selectCategoryFirst'),
    );
    return;
  }
  ...
  try {
    ...
    toast.success(data.message || t('booking.ops.submitSuccessFallback'));
    ...
    if (saveForNextBooking.value) {
      try {
        ...
        toast.warning(t('booking.toastSubmitSaveWarn'));
      }
    }
    ...
    if (e.response?.status === 422) {
      const code = e.response?.data?.error;
      const message = categoryConflictMessage(code, e.response?.data?.message || t('booking.ops.unableSubmit'));
      siteSelectionError.value =
    ...
    toast.error(e.response?.data?.message || t('booking.ops.apiUnavailable'));
  } finally {
