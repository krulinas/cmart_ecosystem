...
const props = defineProps({
  ...
  altText: { type: String, default: 'Reuse item image' },
...
const galleryImages = computed(() => resolveReuseItemGallery(props.item));
...
const syncSelection = () => {
  ...
  const images = galleryImages.value.filter((image) => image.image_url);
  if (!images.length) {
...
watch(() => props.item, syncSelection, { immediate: true, deep: true });
</script>
